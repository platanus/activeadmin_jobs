...
describe "Notifications", type: :feature do
  ...
  context "with multiple jobs" do
    ...
    it "shows multiple notifications", js: true do
      expect(notification_title(1).text).to eq("Finished Job!")
      expect(notification_title(2).text).to eq("Failed Job")

      expect(notification_link(1).text).to eq("2 jobs were completed! :)")
      expect(notification_link(2).text).to eq("the job was not completed :(")

      notification_link(1).click
      expect(page).to have_content "Displaying all 3 Jobs"
    end
  end
end
